...
| 4: #t * #y #H #s #x normalize lapply (H s x)
  ...
  [ #abs destruct
  | #H1 lapply(H' (le_maxr … Hm) H1) #Hn
    change with (max (S y) ?) in match (if ? then ? else ?);
    >max_comm @le_le_max @Hn
  ]
...
lemma four_dot_one_dot_one:
 (∀t.∀s.∀x. (s ≥ fresh_var_t t) → fvb_t x t = fvb x (fst ? ?(underline_pifTerm t s))) ∧
  (∀v.∀s.∀x. (s ≥ fresh_var_tv v) → fvb_tv x v = fvb_v x (fst ? ?(overline v s))).
  
@pifValueTerm_ind
[ #v #HI #s #x #H
  change with (fresh_var_tv v) in match (fresh_var_t ?) in H;
  whd in match (fvb_t ? ?);
  whd in match (free_occ_t x (val_to_term v));
...
| #t * #x #HI #s * #y #H
  ...
  lapply (HI s νy)
  cases (underline_pifTerm t s) #c #n normalize
  #HI' change with (max (S x) (fresh_var_t t) ≤ s) in H;
  lapply (HI' (le_maxr … H)) -HI' #HI' >(neq_simm)
  cases neqb normalize //
] 
