...
let rec hole_subst (t: pTerm) y on t : TermContext  ≝
 match t with
...
let rec p_subst_sig (n: nat) y t': Πt. (t_size t ≤ n) →
 Σu: pTerm. ((t_size u = (t_size t)+ (free_occ_t y t) * (t_size t' - 1) ∧
  (∀z. free_occ_t z u = match veqb y z with
   [ true ⇒ (free_occ_t z t) * (free_occ_t z t')
   | false ⇒ (free_occ_t y t) * (free_occ_t z t') + (free_occ_t z t)
   ]))) ≝
 match n return λn. Πt. (t_size t ≤ n) → Σu: pTerm. ((t_size u = (t_size t)+ (free_occ_t y t) * ((t_size t') - 1)) ∧
  (∀z. free_occ_t z u = match veqb y z with
   [ true ⇒ (free_occ_t z t) * (free_occ_t z t')
   | false ⇒ (free_occ_t y t) * (free_occ_t z t') + (free_occ_t z t)
   ]))
  with
 [ O ⇒ λt.?
 | S m ⇒ λt. match t return λt.  t_size t ≤ S m → Σu: pTerm. ((t_size u = (t_size t)+ (free_occ_t y t) * ((t_size t') - 1)) ∧
    (∀z. free_occ_t z u = match veqb y z with
    [ true ⇒ (free_occ_t z t) * (free_occ_t z t')
    | false ⇒ (free_occ_t y t) * (free_occ_t z t') + (free_occ_t z t)
    ]))
   with
    [ val_to_term v ⇒ match v return λv.  t_size (val_to_term v) ≤ S m → Σu: pTerm. (t_size u = (t_size (val_to_term v))+ (free_occ_v y v) * ((t_size t') - 1) ∧
      (∀z. free_occ_t z u = match veqb y z with
       [ true ⇒ (free_occ_v z v) * (free_occ_t z t')
       | false ⇒ (free_occ_v y v) * (free_occ_t z t') + (free_occ_v z v)
       ]))
      with
     [ pvar x ⇒  match veqb y x return λb.  veqb y x = b → t_size (val_to_term (pvar x)) ≤ S m → Σu: pTerm. (t_size u = (t_size (val_to_term (pvar x)))+ (free_occ_v (match (psubst y t') with [psubst x t ⇒ x]) (pvar x)) * ((t_size (match (psubst y t') with [psubst x t ⇒ t])) - 1) ∧
     ...
     | abstr x t1 ⇒ match veqb y x return λb. veqb y x = b → t_size (val_to_term (abstr x t1)) ≤ S m → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size t') - 1) ∧
        (∀z. free_occ_t z u = match veqb y z with
       ...
       with
        ...
        | false ⇒ λH. match fvb_t x t' return λb. fvb_t x t'=b → t_size (val_to_term (abstr x t1)) ≤ S m → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size t') - 1) ∧
          (∀z. free_occ_t z u = match veqb y z with
            [ true ⇒ (free_occ_v z (abstr x t1)) * (free_occ_t z t')
            | false ⇒ (free_occ_v (y) (abstr x t1)) * (free_occ_t z t') + (free_occ_v z (abstr x t1))
            ]))
         with
          [ true ⇒ λHH. match fvb_t y (val_to_term (abstr x t1)) return λHfvb. fvb_t y (val_to_term (abstr x t1)) = Hfvb →  t_size (val_to_term (abstr x t1)) ≤ S m → Σu: pTerm. ((t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_t y (val_to_term (abstr x t1))) * ((t_size t') - 1)) ∧
            (∀z. free_occ_t z u = match veqb y z with
            ...
            [ true ⇒ λHfv.λp. let z ≝ (max (S match y with [variable n ⇒ n]) (max (S match x with [variable nx⇒ nx]) (max (fresh_var_t t1) (fresh_var_t t'))))
                   in match (p_subst_sig m x (val_to_term (pvar ν(z))) t1 ?) with
               [ mk_Sig a h ⇒ mk_Sig …  (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig m y t' a ?)))) ?]
            | false ⇒ λHfv. λp. mk_Sig pTerm ? (val_to_term (abstr x t1)) ?
            ] (refl …)
          | false ⇒ λHH. λp. mk_Sig … (val_to_term (abstr x (pi1 … (p_subst_sig m y t' t1 ?)))) ?
          ] (refl …)
        ] (refl …)
     ]
    | appl t2 u ⇒  λp. mk_Sig … (appl (pi1 …(p_subst_sig m y t' t2 ? )) (pi1 … (p_subst_sig m y t' u ?))) ?
    ]
 ]
.
[ @(subst_aux_1 …)
...
| @(subst_aux_4 … m H p)
| @(subst_aux_5 … h p)
| @sigma_prop_gen #k #k_def #k_size @(p_subst_aux_6 n y t' m t v x t1 H HH p ? ? a h k k_size) //
| 7,9: @(subst_aux_7 … p)
...
[ *
  [ #x #n' cases n'
    ...
    whd in match (p_subst_sig ? ? ? ? ?);
    whd in match (match psubst y t' in pSubst with [psubst y t'⇒t']);
    cut (∀AAA,BBB,CCC,DDD.
     ((match veqb y x return λb. veqb y x = b → t_size (val_to_term (pvar x))≤S m → Σu:pTerm. ?
       with
    ...
    | * #x #t1 #n'
      ...
      * #y #t' #H #H'
      whd in match (p_subst_sig ? ? ? ? ?);
      whd in match (p_subst_sig (S m') (νy) t' (val_to_term (abstr (νx) t1)) H');
      whd in match (match ? in Variable with [_ ⇒?]);
      whd in match (match ? in Variable with [_ ⇒?]);
      cut (∀AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH.
         (match veqb (νy) (νx) return λb. veqb (νy) (νx) = b → t_size (val_to_term (abstr (νx) t1)) ≤ S m → Σu: pTerm. ?
           with
            [ true ⇒ λH.λp. « (val_to_term (abstr (νx) t1)), AAA H p»
            | false ⇒ λH. match fvb_t (νx) t' return λb. fvb_t (νx) t'=b → t_size (val_to_term (abstr (νx) t1)) ≤ S (m) → Σu: pTerm. ?
             with
              [ true ⇒ λHH. match fvb_t (νy) (val_to_term (abstr (νx) t1)) return λHfvb. fvb_t (νy) (val_to_term (abstr (νx) t1)) = Hfvb →  t_size (val_to_term (abstr (νx) t1)) ≤ S (m) → Σu: pTerm. ?
                with
                [ true ⇒ λHfv.λp. let z ≝ (max (S y) (max (S x) (max (fresh_var_t t1) (fresh_var_t t'))))
                       in match (p_subst_sig (m) (νx) (val_to_term (pvar ν(z))) t1 (subst_aux_7 (νx) t1 m p)) with
                   [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig (m) (νy) t' a (subst_aux_5 … h p))))), BBB H HH Hfv p a h »]
                | false ⇒ λHfv. λp. « (val_to_term (abstr (νx) t1)), CCC H HH Hfv p»
                ] (refl …)
              | false ⇒ λHH. λp. « (val_to_term (abstr (νx) (pi1 … (p_subst_sig (m) (νy) t' t1 (subst_aux_7 (νx) t1 m p))))), DDD H HH p»
              ] (refl …)
            ...
            | false ⇒ λH. match fvb_t (νx) t' return λb. fvb_t (νx) t'=b → t_size (val_to_term (abstr (νx) t1)) ≤ S (m') → Σu: pTerm. ?
             with
              [ true ⇒ λHH. match fvb_t (νy) (val_to_term (abstr (νx) t1)) return λHfvb. fvb_t (νy) (val_to_term (abstr (νx) t1)) = Hfvb →  t_size (val_to_term (abstr (νx) t1)) ≤ S (m') → Σu: pTerm. ?
                ...
                [ true ⇒ λHfv.λp. let z ≝ (max (S y) (max (S x) (max (fresh_var_t t1) (fresh_var_t t'))))
                       in match (p_subst_sig (m') (νx) (val_to_term (pvar ν(z))) t1 (subst_aux_7 (νx) t1 m' p)) with
                   [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig (m') (νy) t' a (subst_aux_5 … h p))))), FFF H HH Hfv p a h »]
                | false ⇒ λHfv. λp. « (val_to_term (abstr (νx) t1)), GGG H HH Hfv p»
                ] (refl …)
              | false ⇒ λHH. λp. « (val_to_term (abstr (νx) (pi1 … (p_subst_sig (m') (νy) t' t1 (subst_aux_7 (νx) t1 m' p))))), HHH H HH p»
              ] (refl …)
            ]  (refl bool (veqb (νy) (νx))) H')
            )
      [ 3: #UUU @UUU | skip ]
      ...
      | cases (fvb_t (νx) t')
        ...
        cases (p_subst_sig m' (νx) …) #a #h
        #AAA #BBB #CCC #DDD #EEE #FFF #GGG #HHH
        change with
        (( « (val_to_term (abstr (ν(?)) (pi1 … (p_subst_sig (m) (νy) t' a (subst_aux_5 … h ?))))), BBB (refl …) (refl …) (refl …) H a h »)
            =
          « (val_to_term (abstr (ν(?)) (pi1 … (p_subst_sig (m') (νy) t' a (subst_aux_5 … h ?))))), FFF (refl …) (refl …) (refl …) H' a h »
        )
        cut (∀KKK,JJJ,LLL,MMM.
         («val_to_term
  ...
  | #t1 #t2 #n' cases n'
    ...
    #y #t' #H1 #H2
    change with (mk_Sig … (appl (pi1 …(p_subst_sig m y t' t1 ? )) (pi1 … (p_subst_sig m y t' t2 ?))) ?)
      in match (p_subst_sig (S m) y t' (appl t1 t2) H1);
    change with (mk_Sig … (appl (pi1 …(p_subst_sig m' y t' t1 ? )) (pi1 … (p_subst_sig m' y t' t2 ?))) ?)
      in match (p_subst_sig (S m') y t' (appl t1 t2) H2);
      >(HI t1 m' y t' ? (subst_aux_10 t1 t2 m' H2))
      >(HI t2 m' y t' … (subst_aux_9 t1 t2 m' H2)) //
] qed.

lemma p_subst_bound_irrelevance:
 ∀n, n', t, y, t', H, H'.
   p_subst_sig n y t' t H = p_subst_sig n' y t' t H'.
#n #n' #y #y #t' #H #H' @p_subst_bound_irrelevance0 qed.

lemma p_subst_distro0:
 ∀n1, n2, t1, t2, y, t', H, H1, H2.
   pi1 … (p_subst_sig (S (n1 + n2)) y t' (appl t1 t2) H) = appl (pi1 … (p_subst_sig n1 y t' t1 H1)) (pi1 … (p_subst_sig n2 y t' t2 H2)).

#n1 #n2 #t1 #t2 #y #t' #H #H1 #H2
whd in match (p_subst_sig (S (n1+n2)) y t' (appl t1 t2) H); @eq_f2  @eq_f
 @p_subst_bound_irrelevance qed.

lemma atomic_subst0: ∀n,y,t',H. pi1 pTerm ? (p_subst_sig n y t' (val_to_term (pvar y)) H) = t'.
@nat_ind
[ #x #t #H @False_ind lapply H /2/ ]
#n #_ #x #t #H
whd in match (p_subst_sig (S n) x t ? H);
cut (∀gg.∀ tt. (pi1 pTerm ?
 (match veqb x x
...
lemma no_subst0: ∀n,x,y,t',H. veqb x y = false → pi1 pTerm ? (p_subst_sig n x t' (val_to_term (pvar y)) H) = (val_to_term (pvar y)).
@nat_ind
...
lemma no_subst20: ∀n,x,t1,t',H.
 pi1 pTerm ? (p_subst_sig n x t' (val_to_term (abstr x t1)) H) = (val_to_term (abstr x t1)).

*
[ #x #t1 #t' #H @False_ind lapply H normalize /2/]
#n #x #t1 #t' #H

whd in match (p_subst_sig (S n) x t' (val_to_term (abstr x t1)) H);

cut (∀K, K1, K2, K3. pi1 pTerm ?
 (match veqb x x return λb. veqb x x = b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. ?
       with
        [ true ⇒ λH.λp. « (val_to_term (abstr x t1)), K H p»
        | false ⇒ λH. match fvb_t x t' return λb. fvb_t x t'=b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. ?
         with
          [ true ⇒ λHH. match fvb_t x (val_to_term (abstr x t1)) return λHfvb. fvb_t x (val_to_term (abstr x t1)) = Hfvb →  t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. ?
            with
            [ true ⇒ λHfv.λp. let z ≝ (max (S match x with [variable n ⇒ n]) (max (S match x with [variable nx⇒ nx]) (max (fresh_var_t t1) (fresh_var_t t'))))
                   in match (p_subst_sig n x (val_to_term (pvar ν(z))) t1 ?) with
               [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig n x t' a (subst_aux_5 … h p))))), K1 H HH Hfv p a h »]
            | false ⇒ λHfv. λp. « (val_to_term (abstr x t1)), K2 H HH Hfv p»
            ] (refl …)
          | false ⇒ λHH. λp. « (val_to_term (abstr x (pi1 … (p_subst_sig n x t' t1 ?)))), K3 H HH p»
          ] (refl …)
...
lemma abstr_step_subst0: ∀n, x, y, t1, t', H1, H2.
 veqb y x =false →
  fvb_t x t' = false →
   (pi1 pTerm ? (p_subst_sig n y t' (val_to_term (abstr x t1)) H1)) =
   (val_to_term (abstr x (pi1 pTerm ? (p_subst_sig n y t' t1 H2)))).
@nat_ind
[ #x #y #t1 #t' #H1 @False_ind lapply H1 normalize /2/ ]
#n #HI * #x * #y #t1 #t' #H1 #H2 #Hveq #Hfv
cut (∀K, K1, K2, K3. pi1 pTerm ?
 (match veqb (νy) (νx) return λb. veqb (νy) (νx) = b → t_size (val_to_term (abstr (νx) t1)) ≤ S (n) → Σu: pTerm. ?
       with
        [ true ⇒ λH.λp. « (val_to_term (abstr (νx) t1)), K H p»
        | false ⇒ λH. match fvb_t (νx) t' return λb. fvb_t (νx) t'=b → t_size (val_to_term (abstr (νx) t1)) ≤ S (n) → Σu: pTerm. ?
         with
          [ true ⇒ λHH. match fvb_t (νy) (val_to_term (abstr (νx) t1)) return λHfvb. fvb_t (νy) (val_to_term (abstr (νx) t1)) = Hfvb →  t_size (val_to_term (abstr (νx) t1)) ≤ S (n) → Σu: pTerm. ?
            with
            [ true ⇒ λHfv.λp. let z ≝ (max (S y) (max (S x) (max (fresh_var_t t1) (fresh_var_t t'))))
                   in match (p_subst_sig (n) (νx) (val_to_term (pvar ν(z))) t1 ?) with
               [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig ? (νy) t' a (subst_aux_5 … h p))))), K1 H HH Hfv p a h »]
            | false ⇒ λHfv. λp. « (val_to_term (abstr (νx) t1)), K2 H HH Hfv p»
            ] (refl …)
          | false ⇒ λHH. λp. « (val_to_term (abstr (νx) (pi1 … (p_subst_sig n (νy) t' t1 ?)))), K3 H HH p»
          ] (refl …)
        ]  (refl bool (veqb (νy) (νx))) H1)
        = val_to_term (abstr (νx) (pi1 pTerm ? (p_subst_sig (S n) (νy) t' t1 H2))))
 [ 5: #UU @UU | 1,2,3: skip] >Hveq  >Hfv #K #K1 #K2 #K3
 change with
 (val_to_term (abstr (νx) (pi1 pTerm ? (p_subst_sig n (νy) t' t1 (subst_aux_7 (νx) t1 n H1))))
 =
 val_to_term (abstr (νx) (pi1 pTerm ? (p_subst_sig (S n) (νy) t' t1 H2))))
  @eq_f @eq_f @eq_f // qed.

lemma no_subst50:
 (∀n,t.∀y.∀t',H. fvb_t (y) t =false → pi1 pTerm ? (p_subst_sig n y t' t H)=t).
@nat_ind
[ *
  [ *
    [ #x #y #t' #H @False_ind lapply H /2/
  ...
  | * #x #t1 * #y #t' #H1 #H2
    ...
    | normalize in H2; >Htf in H2; >if_f #H2
      ...
      * #Hneqb #Hinb
      ...
      cut (∀K, K1, K2, K3. pi1 pTerm (λu:pTerm
     .t_size u
          ...
          | false ⇒ λH. match fvb_t (νx) t' return λb. fvb_t (νx) t'=b → t_size (val_to_term (abstr (νx) t1)) ≤ S (n) → Σu: pTerm. ?
           with
            [ true ⇒ λHH. match fvb_t (νy) (val_to_term (abstr (νx) t1)) return λHfvb. fvb_t (νy) (val_to_term (abstr (νx) t1)) = Hfvb →  t_size (val_to_term (abstr (νx) t1)) ≤ S (n) → Σu: pTerm. ?
              with
              [ true ⇒ λHfv.λp. let z ≝ (max (S y) (max (S x) (max (fresh_var_t t1) (fresh_var_t t'))))
                     in match (p_subst_sig (n) (νx) (val_to_term (pvar ν(z))) t1 (subst_aux_7 (νx) t1 n p)) with
                 [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig (n) (νy) t' a (subst_aux_5 … h p))))), K1 H HH Hfv p a h »]
              | false ⇒ λHfv. λp. « (val_to_term (abstr (νx) t1)), K2 H HH Hfv p»
              ] (refl …)
            | false ⇒ λHH. λp. « (val_to_term (abstr (νx) (pi1 … (p_subst_sig (n) (νy) t' t1 (subst_aux_7 (νx) t1 n p))))), K3 H HH p»
            ] (refl …)
...
| #t1 #t2 #y #t' #H1 #H2
  >(p_subst_bound_irrelevance ? (S ((t_size t1)+(t_size t2))) (appl t1 t2) y t')
  [ 2: normalize //] >p_subst_distro0
  [ 2,3: //]
  @eq_f2
  [
    >(p_subst_bound_irrelevance ? n t1 y t' ? (subst_aux_10 t1 t2 n H1))
    @(HI t1) lapply H2 normalize cases free_occ_t normalize //
  | >(p_subst_bound_irrelevance ? n t2 y t' ? (subst_aux_9 t1 t2 n H1))
    @(HI t2) lapply H2 normalize cases free_occ_t normalize // #n #abs destruct
] qed.

lemma no_subst30:
 (∀n,t, y,t', H. inb_t y t =false →
   (pi1 pTerm ? (p_subst_sig n y t' t H))=t).

lapply fv_to_in_term * #Hin1 #_
#n #t #y #t'#H #H1 @no_subst50
@(bool_impl_inv2 Variable pTerm Variable pTerm inb_t fvb_t y y t t false false) // @Hin1
qed.

lemma fresh_var_p_subst0:
 (∀n, t, y, t', H. inb_t y t=false →
  fresh_var_t (pi1 pTerm ? (p_subst_sig n y t' t H)) ≤ max (fresh_var_t t) (fresh_var_t t')).
...
[ *
  ...
  | * #x #t1 * #y #t' #H1 #H cut (veqb (νy) (νx) = false ∧ (fvb_t (νy) (val_to_term (abstr (νx) t1))=false))
    [ % lapply H normalize cases neqb // normalize #H destruct
      ...
      @(bool_impl_inv2 Variable pTerm Variable pTerm inb_t fvb_t (νy) (νy) t1 t1 false false) //
      @Hintofv
      | * #Hveqb #Hfvb
        change with (max ? (fresh_var_t ?)) in match (fresh_var_tv ?);
        cut (∀K, K1, K2, K3. fresh_var_t (pi1 pTerm ?
        (match veqb (νy) (νx) return λb. veqb (νy) (νx) = b → t_size (val_to_term (abstr (νx) t1)) ≤ S n → Σu: pTerm. ?
         with
          [ true ⇒ λH.λp. « (val_to_term (abstr (νx) t1)), K H p»
          | false ⇒ λH. match fvb_t (νx) t' return λb. fvb_t (νx) t'=b → t_size (val_to_term (abstr (νx) t1)) ≤ S n → Σu: pTerm. ?
      ...
      | #K #K1 #K2 #K3 normalize
        change with (max (S x) ?) in match (if ? then ? else S x);
        change with (p_subst ? (psubst (νy) t')) in match ((pi1 pTerm ?
        (p_subst_sig (t_size t1) (νy) t' t1 (le_n (t_size t1)))));
        change with (fresh_var_t ?) in match (pi1 ? ?
          (fresh_var_t_Sig (pi1 pTerm ? (p_subst_sig ? (νy) t' t1 ?))));
        change with (fresh_var_t t1)
          in match (pi1 ? ? (fresh_var_t_Sig t1 ));
        ...
        |
          cut (∀H. fresh_var_t (pi1 pTerm ? (p_subst_sig n (νy) t' t1 H)) ≤
...
| #t1 #t2 #y #t' #H
  ...
  whd in match (p_subst_sig ? ? ? (appl t1 t2) ?);
  whd in match (fresh_var_t (appl t1 t2));
  change with (fresh_var_t (pi1 pTerm ? (p_subst_sig n y t' t1 (subst_aux_10 t1 t2 n H))))
    in match (pi1 ? ? (fresh_var_t_Sig (pi1 pTerm ?
    (p_subst_sig n y t' t1 (subst_aux_10 t1 t2 n H)))));
  change with (fresh_var_t (pi1 pTerm ? (p_subst_sig n y t' t2 (subst_aux_9 t1 t2 n H))))
    in match (pi1 ? ? (fresh_var_t_Sig (pi1 pTerm ?
    (p_subst_sig n y t' t2 (subst_aux_9 t1 t2 n H)))));
  change with (fresh_var_t t1 )
    in match (pi1 ? ? (fresh_var_t_Sig t1 ));
  ...
  * -Hin #Hin1 #Hin2
  ...
  cut ((max (fresh_var_t (pi1 pTerm ? (p_subst_sig n y t' t1 (subst_aux_10 t1 t2 n H))))
            (fresh_var_t (pi1 pTerm ? (p_subst_sig n y t' t2 (subst_aux_9 t1 t2 n H)))) ≤
...
lemma fv_deletion0:
 (∀n,t,y,w,H. veqb y w = false → fvb_t y (pi1 … (p_subst_sig n y (val_to_term (pvar w)) t H)) = false).
...
| #x #t1 #y #w #Hs #H
       ...
       .free_occ_t z u
        =if veqb y z 
         then free_occ_t z (val_to_term (abstr x t1))
                  ...
                  *free_occ_t z (val_to_term (pvar w))
                  +free_occ_t z (val_to_term (abstr x t1)) ))
  (match veqb y x return λb. veqb y x = b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size (val_to_term (pvar w))) - 1) ∧
        (∀z. free_occ_t z u = match veqb y z with
       ...
       with
        ...
        | false ⇒ λH. match fvb_t x (val_to_term (pvar w)) return λb. fvb_t x (val_to_term (pvar w))=b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size (val_to_term (pvar w))) - 1) ∧
          (∀z. free_occ_t z u = match veqb y z with
            [ true ⇒ (free_occ_v z (abstr x t1)) * (free_occ_t z (val_to_term (pvar w)))
            | false ⇒ (free_occ_v (y) (abstr x t1)) * (free_occ_t z (val_to_term (pvar w))) + (free_occ_v z (abstr x t1))
            ]))
         with
          [ true ⇒ λHH. match fvb_t y (val_to_term (abstr x t1)) return λHfvb. fvb_t y (val_to_term (abstr x t1)) = Hfvb →  t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. ((t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_t y (val_to_term (abstr x t1))) * ((t_size (val_to_term (pvar w))) - 1)) ∧
            (∀z. free_occ_t z u = match veqb y z with
            ...
            [ true ⇒ λHfv.λp. let z ≝ (max (S match y with [variable n ⇒ n]) (max (S match x with [variable nx⇒ nx]) (max (fresh_var_t t1) (fresh_var_t (val_to_term (pvar w))))))
                   in match (p_subst_sig n x (val_to_term (pvar ν(z))) t1 ?) with
               [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig n y (val_to_term (pvar w)) a (subst_aux_5 … h p))))), DDD H HH Hfv p a h »]
            | false ⇒ λHfv. λp. « (val_to_term (abstr x t1)), BBB H HH Hfv p »
            ] (refl …)
          | false ⇒ λHH. λp. « (val_to_term (abstr x (pi1 … (p_subst_sig n y (val_to_term (pvar w)) t1 ?)))), CCC H HH p »
          ] (refl …)
        ]  (refl …) ?))
  =false))
  [ 5: #UU @UU | 1,2,3: skip ]
  ...
  whd in match (match ? in Variable with [_⇒ ?]);
  whd in match (match ? in Variable with [_⇒ ?]);
  cases (p_subst_sig n νnx) [ 2: /2/] #a #h
  cut (∀KKK. (fvb_t (νny)
  (pi1 pTerm ?
   (( «val_to_term  (abstr (ν(?)) (pi1 pTerm ? (p_subst_sig ? (νny)
...
| #t1 #t2 #y #w #Hs #H
  whd in match (p_subst_sig (S n) y (val_to_term (pvar w)) (appl t1 t2) ?);
  ...
  whd in match (fvb_t y ?);
  whd in match (fvb_t y (pi1 pTerm ? (p_subst_sig ? ? ? t1 ?)));
  cases free_occ_t
...
lemma p_subst_plug0:
 (∀n,t,y,t',H. nua_t y t = true → (pi1 … (p_subst_sig n y t' t H)) = plug_t (hole_subst t y) (t')).
@nat_ind
[ *
  [ *
    [ #x #y #w #H @False_ind lapply H normalize /2/
...
| #x #t1 #y #t' #Hs #H
  ...
  | cut (∀ AAA,BBB,CCC,DDD. (eq pTerm
  (pi1 pTerm ?
  (match veqb y x return λb. veqb y x = b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size t') - 1) ∧
        (∀z. free_occ_t z u = match veqb y z with
          [ true ⇒ (free_occ_v z (abstr x t1)) * (free_occ_t z t')
          | false ⇒ (free_occ_v y (abstr x t1)) * (free_occ_t z t') + (free_occ_v z (abstr x t1))
          ]))
       with
        [ true ⇒ λH.λp. « (val_to_term (abstr x t1)), AAA H p»
        | false ⇒ λH. match fvb_t x t' return λb. fvb_t x t'=b → t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. (t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_v y (abstr x t1)) * ((t_size t') - 1) ∧
          (∀z. free_occ_t z u = match veqb y z with
            [ true ⇒ (free_occ_v z (abstr x t1)) * (free_occ_t z t')
            | false ⇒ (free_occ_v (y) (abstr x t1)) * (free_occ_t z t') + (free_occ_v z (abstr x t1))
            ]))
         with
          [ true ⇒ λHH. match fvb_t y (val_to_term (abstr x t1)) return λHfvb. fvb_t y (val_to_term (abstr x t1)) = Hfvb →  t_size (val_to_term (abstr x t1)) ≤ S n → Σu: pTerm. ((t_size u = (t_size (val_to_term (abstr x t1)))+ (free_occ_t y (val_to_term (abstr x t1))) * ((t_size t') - 1)) ∧
            (∀z. free_occ_t z u = match veqb y z with
               [ true ⇒ (free_occ_t z (val_to_term (abstr x t1))) * (free_occ_t z t')
               | false ⇒ (free_occ_t y (val_to_term (abstr x t1))) * (free_occ_t z t') + (free_occ_t z (val_to_term (abstr x t1)))
               ]))
            ...
            [ true ⇒ λHfv.λp. let z ≝ (max (S match y with [variable n ⇒ n]) (max (S match x with [variable nx⇒ nx]) (max (fresh_var_t t1) (fresh_var_t t'))))
                   in match (p_subst_sig n x (val_to_term (pvar ν(z))) t1 ?) with
               [ mk_Sig a h ⇒ « (val_to_term (abstr (ν(z)) (pi1 … (p_subst_sig n y t' a (subst_aux_5 … h p))))), DDD H HH Hfv p a h »]
            | false ⇒ λHfv. λp. « (val_to_term (abstr x t1)), BBB H HH Hfv p »
            ] (refl …)
          | false ⇒ λHH. λp. « (val_to_term (abstr x (pi1 … (p_subst_sig n y t' t1 ?)))), CCC H HH p »
          ] (refl …)
...
| #t1 #t2 #y #t' #Hs
  whd in match (p_subst_sig (S n) y t' (appl t1 t2) ?);
  whd in match (fvb_t y ?);
  whd in match (free_occ_t y ?);
  whd in match (hole_subst ? ?);
  whd in match (plug_t ? ?);
...
definition p_subst ≝ λt.λs. pi1 … (p_subst_sig (t_size t) match s with [psubst y t' ⇒ y] match s with [psubst y t' ⇒ t'] t ?).// qed.
definition p_subst_v ≝ λv.λs. pi1 … (p_subst_sig (t_size (val_to_term v)) match s with [psubst y t' ⇒ y] match s with [psubst y t' ⇒ t'] (val_to_term v) ?).// qed.

lemma p_subst_distro:
 ∀t1, t2, s. p_subst (appl t1 t2) s = appl (p_subst t1 s) (p_subst t2 s).

#t1 #t2 * #y #t'
change with (pi1 … (p_subst_sig (t_size (appl t1 t2)) y t' (appl t1 t2) ?)) in match (p_subst (appl t1 t2) (psubst y t'));
change with (pi1 … (p_subst_sig (t_size t1) y t' t1 ?)) in match (p_subst t1 (psubst y t'));
change with (pi1 … (p_subst_sig (t_size t2) y t' t2 ?)) in match (p_subst t2 (psubst y t'));
...
#x #t
change with (pi1 pTerm ? (p_subst_sig …)) in match (p_subst ? ?);
...
lemma no_subst: ∀y,x,t'.
 veqb x y = false →
  (p_subst (val_to_term (pvar y)) (psubst x t') ) = (val_to_term (pvar y)).
#y #x #t' @no_subst0 qed.
...
lemma abstr_step_subst: ∀x, y, t1, t'.
 veqb y x =false →
  fvb_t x t' = false →
   p_subst (val_to_term (abstr x t1)) (psubst y t') = (val_to_term (abstr x (p_subst t1 (psubst y t')))).

 #x #y #t1 #t' #Hveq #Hfv
 change with (pi1 … (p_subst_sig …)) in match (p_subst (val_to_term (abstr x t1)) (psubst y t'));
whd in match (match ? in pSubst with [_ ⇒ ?]);
whd in match (match ? in pSubst with [_ ⇒ ?]);
...
#t #y #t' #H
change with (pi1 … (p_subst_sig …)) in match (p_subst ? (psubst ? t'));
whd in match (match ? in pSubst with [_ ⇒ ?]);
whd in match (match ? in pSubst with [_ ⇒ ?]);
@no_subst50 @H qed.

lemma no_subst3:
 (∀t,y,t'. inb_t (νy) t =false → p_subst t (psubst (νy) t')=t).
...
lemma fresh_var_p_subst:
 (∀t, y, t'. inb_t y t=false →
  fresh_var_t (p_subst t (psubst y t')) ≤ max (fresh_var_t t) (fresh_var_t t')).

#t #y #t' @fresh_var_p_subst0 qed.

(*
lemma s_psubst:
 (∀t,x,y,t'. fresh_var_t t ≤ y → fresh_var_t t' ≤ y →
   p_subst (p_subst t (psubst x (val_to_term (pvar (νy))))) (psubst (νy) t') =
    p_subst t (psubst x t')) ∧
  (∀v,x,y,t'. fresh_var_tv v ≤ y → fresh_var_t t' ≤ y →
    p_subst (p_subst (val_to_term v) (psubst x (val_to_term (pvar (νy))))) (psubst (νy) t') =
     p_subst (val_to_term v) (psubst x t')).

@pValueTerm_ind
[ #v #H @H
| #t1 #t2 #H1 #H2 * #x #y #t' #H change with (max ? ? ≤y) in H;
  cut (p_subst (appl t1 t2) (psubst (νx) (val_to_term (pvar νy)))=
       appl (p_subst t1 (psubst (νx) (val_to_term (pvar νy))))
            (p_subst t2 (psubst (νx) (val_to_term (pvar νy))))) // #Ht >Ht -Ht
  >p_subst_distro >p_subst_distro #H'
...
lemma p_subst_same_size0:
 (∀n,t,y,w,H.
  t_size (pi1 pTerm ? (p_subst_sig n y (val_to_term (pvar w)) t H)) =
  t_size t).
...
lemma p_subst_plug:
 ∀t, y, t'. nua_t y t = true →
  (p_subst t (psubst y t')) = plug_t (hole_subst t y) (t').
 #t #y #t' @p_subst_plug0 qed.

(*
lemma subst_union0:
 (∀n,t,y,w,t',H1,H2,H3.
  inb_t w t = false →
  pi1 pTerm ? (p_subst_sig n w t' (pi1 pTerm ? (p_subst_sig n y (val_to_term (pvar w)) t H1)) H2) =
   pi1 pTerm ? (p_subst_sig n y t' t H3)).
@nat_ind
[ *
  [ *
...
[ *
  ...
  | #x #t1 #y #w #t' #H1 #H2 #H3 #H
    ...
    [ elim (veqb_true_to_eq y x) #Heq #_ lapply (Heq Hyx) -Heq #Heq destruct
      ...
      cut (∀t:pTerm.∀x0:Variable.inb_t x0 t=false→fvb_t x0 t=false)
    ...
    | >abstr_step_subst0 in H2 ⊢% ; //
      ...
      [ 2: >abstr_step_subst0 in H2 ⊢% ; //
        ...
        | 3: >p_subst_same_size0 lapply H1 normalize /2/
        | 4: whd in match (t_size (val_to_term (abstr x ?)));
          >p_subst_same_size0 lapply H1 normalize /2/ ]
        #H2 >abstr_step_subst0 in H2 ⊢% ; // [ 2: lapply H1 normalize /2/ ]
        #H2 @eq_f @eq_f2 //
        cut (∀K, K1, K2. (pi1 pTerm ?
          (p_subst_sig (S n) w t'
          (pi1 pTerm ? (p_subst_sig (S n) y (val_to_term (pvar w)) t1 K)) K1)
  =pi1 pTerm ?
   (p_subst_sig (S n) y t' t1  K2)))
     [ 2: #UU @UU ] #K #K1 #K2
        >(p_subst_bound_irrelevance ? n t1 y t') [ 2: lapply H1 normalize /2/ ]
        >(p_subst_bound_irrelevance ? n t1 y (val_to_term (pvar w))) in K1 ⊢%;
        [ 2: lapply H1 normalize /2/ ] #K1
        >(p_subst_bound_irrelevance (S n) n ? w t') in K1 ⊢%;
        [ 2: >p_subst_same_size0 lapply H1 normalize /2/
        | 3: >p_subst_same_size0 // ] #K1 @HI lapply H
...
| #t1 #t2 #y #w #t' #H1 #H2 #H3 #H
  whd in match (p_subst_sig ? ? ? (appl t1 t2) ?);
  whd in match (p_subst_sig ? ? ? (appl (pi1 pTerm ? ?) (pi1 pTerm ? ?)) ?);
 ...
 (∀t, u, x, y, z.
  inb_t y t = false →
   inb_t y u = false → veqb y z = false → veqb y x = false →
    veqb x z = false →
       (p_subst (p_subst t (psubst x (val_to_term (pvar y))))
        (psubst z (p_subst u (psubst x (val_to_term (pvar y))))) =
         p_subst (p_subst t (psubst z u)) (psubst x (val_to_term (pvar y))))) ∧
(∀v, u, x, y, z.
  inb_tv y v = false →
   inb_t y u = false →
    veqb y z = false →
     veqb y x = false → veqb x z = false →
       (p_subst (p_subst (val_to_term v) (psubst x (val_to_term (pvar y))))
        (psubst z (p_subst u (psubst x (val_to_term (pvar y))))) =
         p_subst (p_subst (val_to_term v) (psubst z u)) (psubst x (val_to_term (pvar y))))).
...
| #t1 #t2 #Ht1 #Ht2 #u #x #y #z #H1 #H2 #H3 #H4 #H5
  ...
  whd in match (match ? in pSubst with [_⇒?]);
  @eq_f2
  [ >(p_subst_bound_irrelevance ((t_size t1)+(t_size t2)) (t_size t1)) //
    [ 2: //]
     >(p_subst_bound_irrelevance ((t_size t1)+(t_size t2)) (t_size t1)) //
    [ 2: //] >Ht1 //
    lapply H1 normalize cases inb_t normalize //
  | >(p_subst_bound_irrelevance ((t_size t1)+(t_size t2)) (t_size t2)) //
    [ 2: //]
     >(p_subst_bound_irrelevance ((t_size t1)+(t_size t2)) (t_size t2)) //
    [ 2: //] >Ht2 //
...
| #t #x #H #u #w #y #z #H1 #H2 #H3 #H4 #H5
  ...
  [ elim (veqb_true_to_eq x w) #Heq #_ lapply (Heq Hxw) -Heq #Heq destruct
    ...
    whd in match ((p_subst u (psubst w (val_to_term (pvar y)))));
    whd in match (match ? in pSubst with [_⇒?]);
    whd in match (match ? in pSubst with [_⇒?]);
    >abstr_step_subst
    [ 2: lapply (teripp_aux1 (t_size u)) * #tat #_ @tat
    ...
    >H
    whd in match ((p_subst (val_to_term (abstr w t)) (psubst z u)));
    whd in match (match ? in pSubst with [_⇒?]);
    whd in match (match ? in pSubst with [_⇒?]);
    >(abstr_step_subst
    [ 2: lapply (teripp_aux1 (t_size u)) * #tat #_ @tat
    ...
    >(no_subst2 w )
    [
    whd in match (p_subst (val_to_term (abstr w t)) (psubst ? (pi1 …)));
    whd in match (match psubst z (pi1 pTerm ?
                   (p_subst_sig (t_size u) w (val_to_term (pvar y)) u
                    ?))
                   with 
                  [psubst y t⇒t]);
    whd in match (match 
             psubst z
             (pi1 pTerm ?
              (p_subst_sig (t_size u) w (val_to_term (pvar y)) ??))
              with 
             [psubst y t⇒y]);
    whd in match (match ? in pSubst return λ_:pSubst.Variable with [_⇒?]);
    whd in match (match ? in pSubst return λ_:pSubst.Variable with [_⇒?]);
  ...
  whd in match (p_subst (val_to_term (pvar ?)) (psubst ? (val_to_term (pvar ?))));
  whd in match (match ?  in pSubst with [_⇒?]);
  whd in match (match ?  in pSubst with [_⇒?]);
  whd in match ((p_subst_sig (t_size (val_to_term (pvar ?))) ? (val_to_term (pvar ?))
 ...
 cut ( ∀AA,BB. (p_subst
  (pi1 pTerm
   ...
   (match veqb y x return λb.  veqb y x = b → t_size (val_to_term (pvar x)) ≤ 1 → Σu: pTerm. (t_size u = (t_size (val_to_term (pvar x)))+ (free_occ_v y (pvar x)) * ((t_size (val_to_term (pvar w))) - 1) ∧
        (∀z. free_occ_t z u = match veqb y z with
        ...
        with
         [true ⇒λH.λp.«val_to_term (pvar w), AA H p» | false ⇒ λH.λp.«val_to_term (pvar x), BB H p»] (refl …) (le_n (t_size (val_to_term (pvar x)))))))
           (psubst z (p_subst u (psubst y (val_to_term (pvar w)))))
  =p_subst (p_subst (val_to_term (pvar x)) (psubst z u))
