...
| #t1 #t2 cases t2
  [ #v2 cases t1
    ...
    | #u1 #u2 normalize #H1 #H2 #s lapply (H1 s) normalize
      change with (underline_pTerm (appl u1 u2) s) in match (match u2 in pTerm return λ_:pTerm.(Crumble×ℕ) with 
              [val_to_term (v20:pValue)⇒
               match u1 in pTerm return λ_:pTerm.(Crumble×ℕ) with 
               [val_to_term (v1:pValue)⇒
                let 〈vv,n0〉 ≝overline v1 s in 
                let 〈ww,m〉 ≝overline v20 (s+n0) in 〈〈AppValue vv ww,Epsilon〉,m+n0〉
               |appl (u10:pTerm)   (u20:pTerm)⇒
                let 〈c,n0〉 ≝underline_pTerm u1 s in 
                match c in Crumble return λ_:Crumble.(Crumble×ℕ) with 
                [CCrumble (b:Bite)   (e:Environment)⇒
                 let 〈vv,m〉 ≝overline v20 (s+n0) in 
                 〈〈AppValue (var ν(s+n0+m)) vv,push e [ν(s+n0+m)←b]〉,S (s+n0+m)〉]]
              |appl (u10:pTerm)   (u20:pTerm)⇒
               let 〈c,n0〉 ≝underline_pTerm u2 s in 
               match c in Crumble return λ_:Crumble.(Crumble×ℕ) with 
               [CCrumble (b1:Bite)   (e1:Environment)⇒
                ...
                |appl (u100:pTerm)   (u200:pTerm)⇒
                 let 〈c1,n1〉 ≝underline_pTerm u1 (s+n0) in 
  ...
  | #u1 #u2 normalize #H1 #H2 #s lapply (H2 s)
    change with (underline_pTerm (appl u1 u2) ?) in match (match u2 in pTerm return λ_:pTerm.(Crumble×ℕ) with 
              [val_to_term (v2:pValue)⇒
               match u1 in pTerm return λ_:pTerm.(Crumble×ℕ) with 
               [val_to_term (v1:pValue)⇒
                let 〈vv,n0〉 ≝overline v1 s in 
  ...
  | #r1 #r2 cases (underline_pTerm (appl r1 r2) (s+n)) * #b1 #e1 #n1 normalize
